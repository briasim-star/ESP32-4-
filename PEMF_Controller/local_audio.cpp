...
void localaudio_begin() {
  ...
  if (i2s_driver_install(LOCAL_I2S_PORT, &i2sConfig, 0, NULL) != ESP_OK) return;
  i2s_set_pin(LOCAL_I2S_PORT, NULL); // NULL = route to the internal DAC pins (GPIO25/26)
  i2s_set_dac_mode(I2S_DAC_CHANNEL_BOTH_EN); // both channels, even though only GPIO26 is physically wired - avoids the mono swap bug
  i2sInstalled = true;
}
...
void localaudio_update() {
  ...
  if (soundscapePlaying || soundscapeStopping) {
    uint8_t buf[512];
    int bytesRead = soundscapeFile.read(buf, sizeof(buf));
    if (bytesRead <= 0) {
      soundscapeFile.seek(soundscapeDataStart);
      bytesRead = soundscapeFile.read(buf, sizeof(buf));
    }
  ...
  static const int CHUNK = 64; // samples generated per update() call
