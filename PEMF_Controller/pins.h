...
// Uses the ESP32's standard default VSPI pins - genuinely separate from
// the LCD/touch SPI bus above (which uses custom pins 12/13/14), so no
// bus-sharing complexity is needed here.
#define SD_CS      5    // SD card select, low level effective
// SD_MOSI=23, SD_MISO=19, SD_SCLK=18 - these are the ESP32's default VSPI
// pins, used automatically by SD.begin(SD_CS) without needing to specify
// them explicitly.
