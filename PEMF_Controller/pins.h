...
// Uses the ESP32's standard default VSPI pins. Despite using different
// pin *numbers* than the LCD/touch SPI bus above, this can still end up
// sharing the same underlying SPI *hardware peripheral* via the ESP32's
// GPIO matrix - confirmed by real testing (touch stopped responding
// once SD.begin() was added). sd_media.cpp explicitly shares the
// display's own SPI instance (tft.getSPIinstance()) to avoid this.
#define SD_CS      5    // SD card select, low level effective
// SD_MOSI=23, SD_MISO=19, SD_SCLK=18 - these are the ESP32's default VSPI
// pins, used automatically by SD.begin(SD_CS, ...) without needing to
