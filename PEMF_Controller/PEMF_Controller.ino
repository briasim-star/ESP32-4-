...
Rect checkboxRect = {20, 172, 36, 36};
...
void drawBtScanScreen() {
  ...
  tft.setTextDatum(TL_DATUM);
  if (count == 0) {
    tft.drawString((scanning || scanRequested) ? "Looking for nearby devices..." : "No devices found yet - tap Scan.", 20, 92);
  }
...
void drawSequencesScreen() {
  ...
  for (int i = 0; i < SEQUENCES_PER_PAGE; i++) {
    ...
    if (idx < NUM_SEQUENCES) {
      ...
      // gives real information about what a sequence does, not just a
      // name, and gives both lines room instead of cramming everything
      // into one line that gets truncated.
      tft.setFreeFont(FONT_SM);
      tft.setTextColor(TFT_WHITE, COLOR_PANEL);
      tft.setTextDatum(TC_DATUM);
      ...
      tft.drawString(rangeBuf, r.x + r.w / 2, r.y + 32);
      tft.setTextDatum(TL_DATUM);
    }
  }
  ...
  if (totalSeqPages > 1) {
    ...
    tft.setTextColor(COLOR_TEXT_DIM, COLOR_BG);
    tft.setTextDatum(TC_DATUM);
    tft.drawString(pageBuf, 240, 240);
    tft.setTextDatum(TL_DATUM);
  }
}
...
void drawSoundscapesScreen() {
  ...
  if (totalSndPages > 1) {
    ...
    tft.setTextColor(COLOR_TEXT_DIM, COLOR_BG);
    tft.setTextDatum(TC_DATUM);
    tft.drawString(pageBuf, 240, 202);
    tft.setTextDatum(TL_DATUM);
  }
}
...
void drawRunScreen() {
  ...
  if (sessionTimerArmed && running) {
    unsigned long elapsedSec = (millis() - sessionStartMillis) / 1000UL;
    long remainingSec = (long)timerMinutes * 60 - (long)elapsedSec;
    if (remainingSec < 0) remainingSec = 0;
    char timeBuf[24];
    snprintf(timeBuf, sizeof(timeBuf), "Time left: %ld:%02ld", remainingSec / 60, remainingSec % 60);
    tft.setFreeFont(FONT_SM);
...
void setup() {
  ...
  btaudio_setVolume((uint8_t)volumePercent);
  if (strlen(btDeviceName) > 0) {
    // Pre-connect once, here at boot, rather than making the person wait
    // mid-session the first time they toggle BT Audio on. The connection
    // itself persists once established - later on/off toggles during an
    // actual session just mute/unmute generation and are already
    // instant - so this just moves the one real connection cost earlier,
    // before anyone's actively trying to use it.
    tft.fillScreen(COLOR_BG);
    tft.setFreeFont(FONT_LG);
    tft.setTextColor(TFT_WHITE, COLOR_BG);
    tft.setTextDatum(MC_DATUM);
    ...
    tft.setTextColor(COLOR_TEXT_DIM, COLOR_BG);
    tft.drawString("This can take up to a minute.", 240, 175);
    btaudio_setEnabled(true);
    btaudio_setEnabled(false); // connection stays up; just mute generation until a session wants it
  }
#endif
...
void loop() {
  ...
  if (sessionTimerArmed && waveform_isRunning()) {
    ...
    if ((int)elapsedMin >= timerMinutes) {
      ...
      // Live-update the countdown display once a second - without this
      // it would just show whatever value was on screen at the last
      // touch, not actually ticking down.
