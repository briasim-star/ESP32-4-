...
const char* FIRMWARE_VERSION = "1.1.0"; // not static - ota_update.cpp reads this via extern. Bumped from 1.0.0 (unchanged through this entire session despite many real firmware changes) specifically so Settings -> Check for Updates can confirm, from the device itself, whether a given flash actually took effect - check this number after flashing if changes don't seem to be showing up.
