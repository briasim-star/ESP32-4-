...
Screen soundscapesOrigin = SCR_SETTINGS; // where "Back" returns to - SCR_RUN when entered mid-session instead
...
void drawRunScreen() {
  ...
  if (sdmedia_isAvailable() && sdmedia_soundscapeCount() > 0) {
    const char* sndLabel = audio_isSoundscapePlaying()
      ? sdmedia_soundscapeName(nowPlayingSoundscapeIndex)
      : "Choose Soundscape";
