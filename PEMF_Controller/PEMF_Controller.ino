...
void drawWelcomeScreen() {
  ...
  // Small diagnostic line, always shown, no navigation needed - so this
  // can be read directly off the screen instead of needing a computer
  // and Serial Monitor to check the same information.
  char sdDiag[48];
  if (sdmedia_isAvailable()) {
    snprintf(sdDiag, sizeof(sdDiag), "SD card: OK (%d soundscape files found)", sdmedia_soundscapeCount());
  } else {
    snprintf(sdDiag, sizeof(sdDiag), "SD card: not detected");
  }
  tft.setFreeFont(FONT_SM);
  tft.setTextColor(COLOR_TEXT_DIM, COLOR_BG);
  tft.setTextDatum(TL_DATUM);
  tft.drawString(sdDiag, 20, 296);
}
...
Rect btnGames = {260, 64, 200, 48}; // moved up here (from the bottom row) - a frequently-wanted feature, not something to bury
...
Rect btnViewLog = {260, 180, 200, 48}; // moved up here - something to check routinely, not the bottom row
Rect btnSoundscapes = {20, 288, 220, 28};
Rect btnFactoryReset = {260, 288, 200, 28}; // moved down here - a rare, deliberate action, already protected by its own tap-twice confirm
...
void drawSettingsScreen() {
  ...
  if (sdmedia_isAvailable() && sdmedia_soundscapeCount() > 0) {
    ...
    drawButton(btnSoundscapes, sndLabel, COLOR_MUTED, audio_isSoundscapePlaying());
  }
  drawButton(btnGames, "Games", COLOR_MUTED);
}
...
Rect btnHeadphonesToggle = {20, 88, 440, 42};
...
void drawSoundscapesScreen() {
  ...
  drawButton(btnSndBack, audio_isSoundscapePlaying() ? "Stop & Back" : "Back", COLOR_MUTED);
...
void loop() {
  ...
  if (touched && !wasTouched) {
    ...
    if (screen == SCR_CATEGORY)       drawCategoryScreen();
  }
  wasTouched = touched;
