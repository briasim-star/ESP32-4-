...
void drawUpdateScreen() {
  ...
  // Approximate text position below the QR block (version 5 -> 37 modules
  // at scale 5 -> 185px, plus the white margin) - fine as a text position
  // estimate since it doesn't need to be pixel-exact.
  int textY = 118 + (37 * 5) + 26;
  tft.setFreeFont(FONT_SM);
  tft.setTextColor(TFT_WHITE, COLOR_BG);
  tft.setTextDatum(TL_DATUM);
