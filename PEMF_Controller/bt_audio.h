...
void btaudio_setHeadphonesMode(bool on); // true = paired device is headphones (enables true binaural beats); false = a speaker (mono-equivalent isochronic instead)
void btaudio_update(); // call periodically from loop() - finishes the volume-set once a connection is actually confirmed established
bool btaudio_isHeadphonesMode();
