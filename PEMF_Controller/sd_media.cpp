...
bool sdmedia_begin() {
  // Explicitly share the display's own SPI instance (verified pattern
  // from TFT_eSPI's own ESP32 SD-card example) rather than letting SD
  // default to its own SPI object. Different pin *numbers* don't
  // guarantee a separate SPI *peripheral* on the ESP32 - the GPIO matrix
  // can route either pin set to the same underlying hardware controller,
  // and without this, SD and the touch/display can end up silently
  // fighting over the same peripheral (confirmed by real testing: touch
  // stopped responding entirely once the SD code was added).
  sdAvailable = SD.begin(SD_CS, tft.getSPIinstance());
  return sdAvailable;
}
