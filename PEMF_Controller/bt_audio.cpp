...
void btaudio_connectToScanResult(int idx) {
  ...
  if (a2dpSourceStarted) {
    a2dp_source.end();
    delay(400); // give the stack's internal teardown task time to finish
                // before immediately re-starting - end()/start() dispatch
                // to an internal async task, so calling start() right back
                // to back risks a race where the old session isn't fully
                // torn down yet.
  }
  g_started = false;
  a2dp_source.start(g_deviceName, get_sound_data);
