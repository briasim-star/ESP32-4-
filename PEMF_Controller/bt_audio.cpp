...
void btaudio_stopSoundscape() {
  if (soundscapeFile) soundscapeFile.close();
...
int32_t get_sound_data(Frame* data, int32_t frameCount) {
  if (soundscapePlaying) {
    ...
    if (g_volumeFrac < 0.999f) {
      for (int32_t i = 0; i < frameCount; i++) {
        data[i].channel1 = (int16_t)(data[i].channel1 * g_volumeFrac);
        data[i].channel2 = (int16_t)(data[i].channel2 * g_volumeFrac);
      }
    }
    return frameCount;
  }
